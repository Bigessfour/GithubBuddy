...
function printSecurityGuidance() {
  ...
  console.log('1. Use a fine-grained Personal Access Token (PAT) with the minimum required scopes (Contents: Read-only).');
  ...
  console.log('  PATs: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token');
  console.log('  SSH:  https://docs.github.com/en/authentication/connecting-to-github-with-ssh\n');
}
...
async function runInteractive() {
  ...
  const url = await prompt(`Enter the upstream repo URL (default: ${DEFAULT_UPSTREAM}): `) || DEFAULT_UPSTREAM;

  if (fs.existsSync(TARGET_DIR)) {
    console.log(`\nTarget directory already exists: ${TARGET_DIR}`);
    const overwrite = await prompt('Do you want to remove it and re-clone? (y/N): ');
  ...
  try {
    ...
    console.log('The app will now load full day focus content from the upstream materials.\n');
  } catch (err) {
    console.error('\n❌ Clone failed. Please check your authentication and try again.');
    console.error('You can run this script again anytime with: npm run setup-course\n');
  }
}
...
async function runAuto() {
  ...
  const answer = await prompt('Would you like to clone the upstream repo now? (Y/n): ');
  if (answer.toLowerCase() === 'n') {
    console.log('You can run `npm run setup-course` later to complete the setup.');
    return;
  }
  ...
  try {
    execSync(`git clone ${url} ${TARGET_DIR}`, { stdio: 'inherit' });
    console.log('✅ Upstream repo ready. Full day focus feature is now active.');
  } catch (err) {
